...
std::string NetworkClient::GetServerInfo() const {
    if (connected_) { return server_host_ + ":" + std::to_string(server_port_); }
...
void NetworkClient::HandleRead() {
    ...
    if (len > 0) {
        ...
        if (receive_callback_) { receive_callback_(data.data(), len); }
    }
}
...
void NetworkClient::HandleEvent(short events) {
    if (events & BEV_EVENT_CONNECTED) {
        ...
        // Cleanup(only bev need cleanup here)
        DestroyClientBev();
    }
}

void NetworkClient::DestroyClientBev() {
    if (bev_) {
        bufferevent_free(bev_);
        bev_ = nullptr;
    }
