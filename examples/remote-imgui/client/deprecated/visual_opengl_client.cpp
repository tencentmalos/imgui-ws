// Visual OpenGL client for remote ImGui with actual window rendering
...
VisualOpenGLClient::VisualOpenGLClient() {
    window_ = nullptr;
...
bool VisualOpenGLClient::initialize(int window_width, int window_height, const char* title) {
    ...
    initialized_ = true;
    std::cout << "Visual OpenGL client initialized successfully" << std::endl;
    return true;
}

bool VisualOpenGLClient::connect(const std::string& server_ip, int port) {
...
bool VisualOpenGLClient::setupGLFWWindow() {
    // Setup GLFW
...
bool VisualOpenGLClient::setupOpenGL() {
...
void VisualOpenGLClient::cleanup() {
    if (window_) {
    ...
    glfwTerminate();
    std::cout << "Visual OpenGL client cleaned up" << std::endl;
}

void VisualOpenGLClient::setupImGui() {
...
void VisualOpenGLClient::renderFrame() {
...
void VisualOpenGLClient::renderImGui() {
    // Main menu bar
    ...
    // Main window
    {
        ImGui::Begin("Remote ImGui Client - Visual OpenGL Rendering");

        ImGui::Text("This is a visual OpenGL-based ImGui client");
        ImGui::Text("Window size: %dx%d", window_width_, window_height_);

        // Get current framebuffer size
        int display_w, display_h;
        glfwGetFramebufferSize(window_, &display_w, &display_h);
        ImGui::Text("Framebuffer size: %dx%d", display_w, display_h);
        ...
        if (show_about) {
            ...
            ImGui::Text("Built with ImGui official examples");
    ...
    if (show_data_window_) {
        ...
        if (!received_data_.empty()) {
            if (ImGui::BeginChild("Hex View", ImVec2(0, 200), true)) {
                ImGui::Text("Hex view (first 128 bytes):");

                for (size_t i = 0; i < std::min((size_t)128, received_data_.size()); i += 16) {
                    // Offset
                    ImGui::Text("%04zx: ", i);
                    ImGui::SameLine();
...
void VisualOpenGLClient::run() {
    if (!initialized_ || !window_) {
        std::cerr << "Client not initialized!" << std::endl;
        return;
    }

    std::cout << "Starting visual OpenGL ImGui client..." << std::endl;
    std::cout << "Window created - you should see the ImGui interface" << std::endl;
    ...
    std::cout << "Visual OpenGL client main loop ended" << std::endl;
}

int main(int, char** argv) {
    VisualOpenGLClient client;

    // Initialize client
    if (!client.initialize(1280, 720, "Remote ImGui Client - Visual OpenGL Rendering")) {
        std::cerr << "Failed to initialize visual OpenGL client!" << std::endl;
        return -1;
    }
