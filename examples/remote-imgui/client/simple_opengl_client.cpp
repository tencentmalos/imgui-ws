...
SimpleOpenGLClient::SimpleOpenGLClient() {
    ...
    if (result != 0) { std::cerr << "WSAStartup failed with error code: " << result << std::endl; }
#endif
}

SimpleOpenGLClient::~SimpleOpenGLClient() { Cleanup(); }
...
bool SimpleOpenGLClient::InitializeNetwork() {
    // Create network processor if not exists
    if (!network_processor_) {
        network_processor_ = std::make_unique<spatial::debugger::NetPacketDispatcher>();

        // Set packet handler
        network_processor_->BindServicePacketHandler(
            spatial::debugger::NetServiceType::RemoteImgui,
            [this](spatial::debugger::NetServiceType service_type, uint16_t service_cmd,
                const spatial::debugger::NetPacketBuffer& packet) {
                    if (service_cmd ==
                        static_cast<uint32_t>(spatial::debugger::ImGuiCommand::FrameData)) {
                        // Process ImGui draw data
                        if (deserializer_.DeserializePacket(packet)) {
                            std::cout << "Received and deserialized ImGui frame data: "
                                << packet.TotalSize() << " bytes" << std::endl;
                        ...
                        static_cast<uint32_t>(
                            spatial::debugger::ImGuiCommand::FontTexture)) {
                        // Process font texture data
                        if (deserializer_.DeserializeFontTexture(packet)) {
                            std::cout << "Received and deserialized font texture data: "
                                << packet.TotalSize() << " bytes" << std::endl;
                        ...
                        std::cout << "Received other service data: " << (int)service_type << " "
                            << ", cmd: " << service_cmd << ", size: " << packet.TotalSize()
                            << " bytes" << std::endl;
                    }
            });
    }
    ...
    return network_client_->IsIntialized();
}
...
void SimpleOpenGLClient::RenderServerLikeUI() {
    ...
    ImGui::Text("Connected clients: 1");
    ...
    // Render local UI
    ImGui::Render();
    if (ImGui::GetDrawData() != nullptr) { ImGui_ImplOpenGL3_RenderDrawData(ImGui::GetDrawData()); }
...
void SimpleOpenGLClient::RenderLocalUI() {
    ...
    if (show_network_window_) {
        ...
        if (ImGui::Button("Connect")) { Connect(server_ip, server_port); }
        ImGui::SameLine();
...
void SimpleOpenGLClient::RenderRemoteFrameOnly() {
    ...
    std::cout << "Rendering remote frame data with " << remote_draw_data.CmdListsCount
              << " draw lists" << std::endl;
...
std::vector<uint8_t> SimpleOpenGLClient::ConvertAlphaToRgbaWhite(const uint8_t* alpha_data,
                                                                 int width, int height) {
    if (!alpha_data || width <= 0 || height <= 0) { return {}; }
...
void SimpleOpenGLClient::UpdateFontTexture(const uint8_t* font_data, int width, int height) {
    if (font_data == nullptr || width <= 0 || height <= 0) {
        std::cerr << "Invalid font texture data: " << (void*) font_data << ", " << width << "x"
                  << height << std::endl;
        return;
    }
    ...
    for (int i = 0; i < std::min(16, width * height); i++) { printf("%02X ", font_data[i]); }
    std::cout << std::endl;
    ...
    if (data_matches) {
        ...
        for (int i = 0; i < std::min(8, width * height); i++) { printf("%02X ", font_data[i]); }
        std::cout << std::endl;
