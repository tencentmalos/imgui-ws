...
void ImDrawDataSerializer::setDrawData(ImDrawData* draw_data) {
    ...
    if (serialized_data_.size() >= sizeof(FrameHeader)) {
        *reinterpret_cast<uint32_t*>(&serialized_data_[8]) = static_cast<uint32_t>(serialized_data_.size());
    }

    std::cout << "Serialized ImGui frame: " << draw_data->CmdListsCount
              << " draw lists, total size: " << serialized_data_.size() << " bytes" << std::endl;
}

void ImDrawDataSerializer::serializeHeader(const ImDrawData* draw_data) {
    // Write frame header
    writeUint32(FRAME_MAGIC);
...
void ImDrawDataSerializer::serializeDrawList(const ImDrawList* draw_list) {
    ...
    for (int i = 0; i < draw_list->CmdBuffer.Size; i++) {
        ...
        serialized_cmd.clip_rect[0] = static_cast<uint32_t>(cmd.ClipRect.x * 1000.0f); // Convert to integer storage
        serialized_cmd.clip_rect[1] = static_cast<uint32_t>(cmd.ClipRect.y * 1000.0f);
        serialized_cmd.clip_rect[2] = static_cast<uint32_t>(cmd.ClipRect.z * 1000.0f);
