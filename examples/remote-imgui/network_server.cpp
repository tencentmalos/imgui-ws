...
bool NetworkServer::sendToClient(int client_id, const uint8_t* data, size_t size) {
...
void NetworkServer::handleNewConnection(evutil_socket_t fd, sockaddr* addr, int socklen) {
    ...
    std::cout << "Client connected: " << client_id << " from " << address << std::endl;
...
NetworkServer::ClientConnection* NetworkServer::findClientByBufferevent(bufferevent* bev) {
    for (auto& client : clients_) {
        if (client->bev == bev) {
            return client.get();
        }
