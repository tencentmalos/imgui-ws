...
bool ImDrawDataDeserializer::deserializePacket(const uint8_t* data, size_t size) {
    if (!data || size == 0) {
    ...
    std::cout << "Successfully deserialized ImGui frame: "
              ...
              << current_frame_->draw_commands.size() << " draw commands" << std::endl;

    return validateData();
}

bool ImDrawDataDeserializer::parseFrameHeader(const std::vector<uint8_t>& data, size_t size, size_t& offset) {
    ...
    if (current_frame_->header.magic != FRAME_MAGIC) {
        std::cerr << "Invalid magic number: 0x" << std::hex << current_frame_->header.magic << std::endl;
        return false;
    }

    if (current_frame_->header.version != PROTOCOL_VERSION) {
        std::cerr << "Unsupported protocol version: " << current_frame_->header.version << std::endl;
        return false;
    }
...
bool ImDrawDataDeserializer::parseDrawLists(const std::vector<uint8_t>& data, size_t size, size_t& offset) {
    // Clear previous data
    ...
    for (uint32_t i = 0; i < current_frame_->header.cmd_lists_count; i++) {
        ...
        if (list_header.vtx_buffer_size > 0) {
            ...
            current_frame_->vertex_buffers.insert(current_frame_->vertex_buffers.end(),
